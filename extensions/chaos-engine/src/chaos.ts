...
export class LorenzAttractor {
  ...
  /**
   * Shift sigma based on current engine phase.
   * Idle = tighter orbits, Build = standard, Drop = wide chaotic orbits.
   */
  updatePhase(phase: Phase): void {
    switch (phase) {
      case Phase.Idle:  this.sigma = this.baseSigma * 0.8; break;   // σ=8
      case Phase.Build: this.sigma = this.baseSigma;       break;   // σ=10
      case Phase.Drop:  this.sigma = this.baseSigma * 1.4; break;   // σ=14
    }
  }

  /**
   * Normalized output: x ∈ [-20, 20] → [0, 1]
   * Used to reseed the logistic map.
   */
  ...
  /**
   * Apply transient cognitive noise from incubating thoughts.
   * Adds a perturbation to sigma that decays naturally.
   */
   ...
   * This permanently reshapes the attractor's orbital topology.
   */
...
export class LogisticMap {
  ...
  constructor(seed: number = 0.506) {
    // Ensure seed is in valid range (0, 1), avoid fixed points
    this.val = clamp(seed, 0.01, 0.99);
  }

  /**
   * Advance one iteration and return the new value ∈ (0, 1).
   */
  nextVal(): number {
    this.val = LOGISTIC_R * this.val * (1.0 - this.val);
    // Clamp to avoid numerical drift to 0 or 1
    this.val = clamp(this.val, 0.001, 0.999);
    return this.val;
  }

  /**
   * Reseed from Lorenz normalized output. Called every 10 ticks.
   * Couples the two chaos sources.
   */
  reseedFromLorenz(normalized: number): void {
    // Blend 50/50 to prevent total phase-lock
