...
export class WikiEngine {
  ...
  private saveDigest(): void {
    Bun.write(this.digestPath, JSON.stringify(this.digest, null, 2)).catch(() => {});
  }
...
5. Is concise but complete`;
      ...
      try {
        ...
        await Bun.write(wikiFilePath, article);
...
6. Data flow diagram (as a text description)`;
    ...
    try {
      ...
      console.log(`[WIKI] Self-documented: wiki/entities/${filename} (${article.length} chars)`);
